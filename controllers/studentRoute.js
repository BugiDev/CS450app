...
module.exports = function (app, permissionMiddleware) {
    ...
    app.post('/student/updatePreexamPoints', permissionMiddleware.adminApproved, function (req, res, next) {
        if (!req.body.id) {
            logger.error('You have to provide id for updating preexam points for a student!');
            res.status(404);
            res.json('You have to provide id for updating preexam points for a student!');
        } else if (!req.body.preexamPoints) {
            logger.error('You have to provide preexamPoints for updating preexam points for a student!');
            res.status(404);
            res.json('You have to provide preexamPoints for updating preexam points for a student!');
        } else {
            Student.updatePreexamPoints(req.body.id, req.body.preexamPoints).then(
                function (data) {
                    logger.info('Updating preexam points for a student: ' + data._id);
                    res.json(data);
                }, function (err) {
                    logger.error(err);
                    res.json(err);
    ...
    app.post('/student/updateExamPoints', permissionMiddleware.adminApproved, function (req, res, next) {
        if (!req.body.id) {
            logger.error('You have to provide id for updating exam points for a student!');
            res.status(404);
            res.json('You have to provide id for updating exam points for a student!');
        } else if (!req.body.examPoints) {
            logger.error('You have to provide examPoints for updating exam points for a student!');
            res.status(404);
            res.json('You have to provide examPoints for updating exam points for a student!');
        } else {
            Student.updateExamPoints(req.body.id, req.body.examPoints).then(
                function (data) {
                    logger.info('Updating exam points for a student: ' + data._id);
                    res.json(data);
                }, function (err) {
                    logger.error(err);
                    res.json(err);
    ...
    app.post('/student/resetPassword', permissionMiddleware.adminApproved, function (req, res, next) {
        if (!req.body.id) {
            logger.error('You have to provide id for resetting password for a student!');
            res.status(404);
            res.json('You have to provide id for resetting password for a student!');
        } else {
            Student.resetPassword(req.body.id).then(
                function (data) {
                    logger.info('Reset password for a student: ' + data._id);
                    res.json(data);
                }, function (err) {
                    logger.error(err);
                    res.json(err);
